...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  # above.                                                                     #
  ##############################################################################
  N = x.shape[0]; T = x.shape[1]; H=h0.shape[1]
  h = np.zeros((N,T,H))
  # cache will be the output of the last step
  cache = list()
  for i in range(x.shape[1]):
    h_next, cache_ = rnn_step_forward(x[:,i,:], h0, Wx, Wh, b)
    h0=h_next
    h[:,i,:]=h_next
    cache.append(cache_)
    #h[:,i,:] = h_next
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  #############################################################################
  N,H = prev_h.shape
  a_all = np.dot(x,Wx) + np.dot(prev_h, Wh) + b
  a_i = a_all[:,:H]; a_f=a_all[:,H:2*H]; a_o = a_all[:,2*H:3*H]; a_g=a_all[:,3*H:4*H]
  i = sigmoid(a_i)
  f = sigmoid(a_f)
  o = sigmoid(a_o)
  g = np.tanh(a_g)
